...
export async function  addNewsSchedule(req: any, res: Response) {
    const {startTime, endTime, weeks, campus, teacher1="",teacher2="", content="", classes="", classroom="", updatePRPackage="", remarks="",auditions_num="",age_range=""} = req.body;
    connection.query(
...
export async function  editSchedule(req: any, res: Response) {
    // 根据id进行删除,然后重新插入数据
    const { id ,startTime, endTime, weeks, teacher1="",teacher2="", content="", classes="", classroom="", updatePRPackage="", remarks="", auditions_num="",age_range=""} = req.body;
    connection.query(
        `update schedules set startTime = ?, endTime = ?, weeks = ?, teacher1 = ?, teacher2 = ?, 
content = ?, classes = ?, classroom = ?, updatePRPackage = ?, remarks = ?, auditions_num = ?, age_range = ? where id = ?`,
        [startTime, endTime, weeks, teacher1, teacher2, content, classes, classroom, updatePRPackage, remarks, auditions_num,age_range, id ],
        async function (err){
            if (err) {
                Logger.error(err);
                await res.json({
...
export async function  useLastWeekSchedule(req: any, res: Response) {
    ...
    const insertFun = (res) => {
        const {startTime, endTime, weeks, campus, teacher1="",teacher2="", content="", classes="", classroom="", updatePRPackage="", remarks="", auditions_num="", age_range=""} = res;
        connection.query(
            `insert into schedules (startTime, endTime, weeks, campus, teacher1, teacher2, content, classes, classroom, updatePRPackage, remarks, auditions_num, age_range) values (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
            [startTime, endTime, weeks, campus, teacher1, teacher2, content, classes, classroom, updatePRPackage, remarks, auditions_num, age_range]
        );
